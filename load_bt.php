...
//        if ($check_login == "TRUE") {
		...
		while ($row = $result->fetch_assoc()) {
			...
			$html .= "<tr><td><a href=\"index.php?action=Bidability&ProjectID=$row[ProjectID]\">$row[ProjectName]</a></td><td>$row[PayItem]</td></tr>";
			$found++;
		}
		// show list
		...
		if ($found == "1") {
			...
			?>
			<script>
				document.location.href='index.php?action=Bidability&ProjectID=<?=$ProjectID;?>';
			</script>
			<?
