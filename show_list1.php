...
if ($_SESSION['ct'] == "Gl9dW3jck") {
        ...
        if ($check_login == "TRUE") {
			...
			$sql = "SELECT * FROM `DesignConsultantContact` GROUP BY `DesignConsultant` ORDER BY `DesignConsultant` ASC";
			...
			while ($row = $result->fetch_assoc()) {
				...
				$options .= "<option $selected value=\"$row[ConsultantID]\">$row[DesignConsultant]</option>";
			}
        ...
        <input type="hidden" name="section" value="addto">
        <input type="hidden" name="ProjectID" value="<?=$_GET['ProjectID'];?>">
        <input type="hidden" name="part" value="designer">
		
	
		        <table border=0 width=100%>
	        	<tr><td align=right>Organization</td><td><select name="DesignConsultant" onchange="load_list1(this.form)"><?=$options;?></select>  <?=$hidden;?>
	                </td></tr>
        	        <tr><td align=right>Contact Person</td><td><input type="text" name="ContactName" value="<?=$ContactName;?>" size=20>
