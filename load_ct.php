...
if ($_SESSION['ct'] == "Gl9dW3jck") {
		...
		while ($row = $result->fetch_assoc()) {
			...
			$html .= "<tr><td><a href=\"index.php?action=contacts&type=load&ProjectID=$row[ProjectID]\">$row[ProjectName]</a></td><td>$row[PayItem]</td></tr>";
			$found++;
		}
		// show list
		...
		if ($found == "1") {
			...
			?>
			<script>
				document.location.href='index.php?action=contacts&type=load&ProjectID=<?=$ProjectID;?>';
			</script>
			<?
